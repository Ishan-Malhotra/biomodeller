...
import { ELEMENT_COLOR } from './atomStyle.ts'
...
export function Depiction2D({ residues, theme }: { residues: readonly Residue[]; theme: Theme }) {
        ...
        >
          ...
          <g className="atoms">
            {placed.map(({ node, cx, cy }) => (
              <g key={node.key} className={node.isRealAtom ? 'atom' : 'atom cap'}>
                {/* Unlabelled carbons are bare vertices, drawn small — the skeletal
                    convention, and what keeps an aromatic ring legible. */}
                <circle
                  cx={cx}
                  cy={cy}
                  r={node.showLabel ? nodeRadius : nodeRadius * 0.4}
                  fill={colorOf[node.element]}
                />
                {node.showLabel && (
