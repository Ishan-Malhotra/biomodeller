 ...
 * previous residue list to the new one, and `rebuildFrom` reuses the untouched
 * prefix atoms *by reference*. Two things follow, both of which matter to the
 * feel of the app: long chains stay responsive keystroke-to-keystroke, and the
 * part of the structure the user didn't touch is byte-identical rather than
 * recomputed to the same value, so nothing shimmers as they type.
 *
...
import { firstChangedIndex, rebuildFrom } from '../lib/chain.ts'
import {
  appendResidue,
  duplicateResidue,
  insertResidue,
...
export function useChain(initial: readonly Residue[] = []): ChainEditor {
  ...
  const cache = useRef<Cache>({ residues: [], atoms: [], fromIndex: 0 })
  ...
  const { atoms, fromIndex } = useMemo(() => {
    ...
    const next = rebuildFrom(cache.current.atoms, residues, from)
    cache.current = { residues, atoms: next, fromIndex: from }
